...
RUN cd nginx-1.31.5 && ./configure --with-compat --add-dynamic-module=../ngx_brotli \
    && make modules

FROM nginx:1.31.5
COPY --from=builder /app/nginx-1.31.5/objs/ngx_http_brotli_static_module.so /etc/nginx/modules/
COPY --from=builder /app/nginx-1.31.5/objs/ngx_http_brotli_filter_module.so /etc/nginx/modules/
